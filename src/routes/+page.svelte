<script lang="ts">
  ...
  let relays = (
    $page.url.searchParams.get("relays") ?? "wss://relay-jp.nostr.wirednet.jp"
  ).replace(",", "\n");
  let result = "";
...
<div class="max-width mx-auto py-4">
  <div class="mb-3">
    ...
    <input type="text" class="form-control" bind:value={tableName} />
  </div>
  <div class="mb-3">
    <label for="" class="form-label">author hex:</label>
    <input type="text" class="form-control" bind:value={authorHex} />
  </div>
  <div class="mb-3">
    <label for="" class="form-label">relays:</label>
    <textarea class="form-control" bind:value={relays} />
  </div>
  ...
  <textarea class="form-control result" bind:value={result} />
</div>
